...
class Sine(tkinter.Frame):
    pi = 3.1415926535
    def __init__(self, master, arow, period_callback, phase_callback, xml_sine):
    ...
    def create_widgets(self):
        ...
        self.rowconfigure(index=self.__the_row, weight=1)
        self.period_label = tkinter.ttk.Label(self, text="Period")
        self.period_label.grid(row=self.__the_row, column=0, sticky=NSEW)
        # TCL book p 509
        ...
        self.period_entry.grid(row=self.__the_row, column=1, sticky=NSEW)
        self.period_entry.bind('<Key-Return>', self.period_callback)
        self.period_entry.bind('<FocusOut>', self.period_callback)
        self.__the_row = self.__the_row + 1

        self.rowconfigure(index=self.__the_row, weight=1)
        self.phase_scroll_label = tkinter.ttk.Label(self, text="-pi..................0..................pi")
        self.phase_scroll_label.rowconfigure(index=self.__the_row, weight=1)
        self.phase_scroll_label.grid(row=self.__the_row, column=1, sticky=NSEW)
        self.__the_row = self.__the_row + 1

        self.rowconfigure(index=self.__the_row, weight=1)
        self.phase_label = tkinter.ttk.Label(self, text="Phase")
        self.phase_label.rowconfigure(index=self.__the_row, weight=1)
        self.phase_label.grid(row=self.__the_row, column=0, sticky=NSEW)
        self.phase_scrollbar = tkinter.ttk.Scrollbar(self, command=self.phase_callback)
        self.phase_scrollbar["orient"] = "horizontal"
        self.phase_scrollbar.grid(row=self.__the_row, column=1, sticky=NSEW)
        self.phase_scrollbar.rowconfigure(index=self.__the_row, weight=1)
    ...
    def install_xml_sine(self, xml_sine):
        ...
        self.update()
#    def validate_length(proposed_value, adddel, cur_value, change_index, action):
    def validate_length(self, d, i, P, s, S, v, V, W):
        # 1-insert, 0=del, -1 if forced
        #pat = re.compile('[a-gA-G]([#bx]|bb)-?[[:digit:]]')
        pat = re.compile('\d+([.]\d*)?')
        ma = pat.match(str(P))
        ret = False
        if (ma):
